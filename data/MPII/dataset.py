...
from lib.utils.func_utils import load_img, get_bbox_pennaction
...
class MPII(Dataset):
    def __init__(self, transform, data_split):
        ...
        self.root_path = root_path = os.path.join('data', 'MPII')
    ...
    def __getitem__(self, index):
        ...
        orig_img_path = os.path.join(self.data_dir, 'images', f'{sample_id}.jpg')
        ...
        # 2D keypoint
        keypoint_path = os.path.join(self.data_dir, 'keypoints', f'{sample_id}.npy')
        keypoints_2d_body_data = np.load(keypoint_path)
 
        keypoints_2d_body = keypoints_2d_body_data[:, :2]
        keypoints_2d_body_valid = keypoints_2d_body_data[:, 2]

        bbox_foot_l, bbox_foot_r = get_bbox_pennaction(keypoints_2d_body, keypoints_2d_body_valid, r_ankle_idx=self.r_ankle_idx, l_ankle_idx=self.l_ankle_idx, foot_frac=0.4, image_size=(img_h, img_w))

        # Contact Annotation
        save_contact_l_label_path = os.path.join(self.data_dir, f'contacts', f'{sample_id}_l.npy')
        save_contact_r_label_path = os.path.join(self.data_dir, f'contacts', f'{sample_id}_r.npy')
        os.makedirs(os.path.dirname(save_contact_l_label_path), exist_ok=True)
        os.makedirs(os.path.dirname(save_contact_r_label_path), exist_ok=True)


        contact_f = np.zeros(265, dtype=int)
        contact_f_l_joint_openpose_2d = np.load(save_contact_l_label_path).astype(np.float64)
