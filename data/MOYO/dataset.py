...
class MOYO(Dataset):
    def __init__(self, transform, data_split, task=None):
        ...
        self.root_path = root_path = os.path.join('data', 'MOYO')
    ...
    def __getitem__(self, index):
        aid = self.db[index]
        if len(aid.split('-')) == 4:
            seq_name = aid.split('-')[0] + '-' + aid.split('-')[1]
            camera_name = aid.split('-')[2]
            img_name = aid.split('-')[3]
        elif len(aid.split('-')) == 7:
            seq_name = aid.split('-')[0] + '-' + aid.split('-')[1] + '-' + aid.split('-')[2]
            camera_name = aid.split('-')[3]
            img_name = aid.split('-')[4] + '-' + aid.split('-')[5] + '-' + aid.split('-')[6]
        elif len(aid.split('-')) == 9:
            seq_name = aid.split('-')[0] + '-' + aid.split('-')[1] + '-' + aid.split('-')[2] + '-' + aid.split('-')[3]
            camera_name = aid.split('-')[4]
            img_name = aid.split('-')[5] + '-' + aid.split('-')[6] + '-' + aid.split('-')[7] + '-' + aid.split('-')[8]
        elif len(aid.split('-')) == 11:
            seq_name = aid.split('-')[0] + '-' + aid.split('-')[1] + '-' + aid.split('-')[2] + '-' + aid.split('-')[3]  + '-' + aid.split('-')[4]
            camera_name = aid.split('-')[5]
            img_name = aid.split('-')[6] + '-' + aid.split('-')[7] + '-' + aid.split('-')[8] + '-' + aid.split('-')[9] + '-' + aid.split('-')[10]
        else:
            seq_name = aid.split('-')[0] + '-' + aid.split('-')[1]
            camera_name = aid.split('-')[2]
            img_name = aid.split('-')[3] + '-' + aid.split('-')[4]
        sample_id = f'{seq_name}-{camera_name}-{img_name}'


        orig_img_path = os.path.join(self.data_dir, seq_name, camera_name, f'{img_name}.jpg')
